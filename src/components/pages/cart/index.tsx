...
import { addToCart, clearCart, decreaseQuantity, ICartItem, removeFromCart, useCartItems, useTotalPrice } from "@/redux/features/cart/cartSlice";
...
const cartItems=useAppSelector(useCartItems);
  return (
   <div className="min-h-screen  mx-auto lg:gap-36 flex flex-col lg:flex-row mb-16 px-6 justify-between items-center lg:items-start mt-6">
    ...
    <h3 className="font-bold text-2xl lg:text-3xl">An overview of your order</h3>
    <button onClick={()=>dispatch(clearCart())} className="bg-blue-50 px-5 rounded-full border active:scale-95">Clear All</button>
    </div>
    <div>
      {
        cartItems.length===0?
...
<button onClick={()=>dispatch(addToCart({product}))} className="w-10 text-3xl active:scale-95 bg-gray-100 flex justify-center items-center"><span>+</span></button>
   ...
   <Image src={product.image} height={20} width={20} alt={product.name}  className="md:w-20 md:h-20 w-10 h-10 rounded-2xl" />
 </div>
<div className="flex flex-col justify-start item-start">
<p className="font-semibold ml-8">{product.name}</p>
<p className="text-sm ml-8"><span className="font-medium">expiry date:</span> {new Date(product.expiryDate).toLocaleDateString()}</p>
</div>
</div>
  <div className="flex flex-col justify-between items-end">
    ...
    <section className="w-full lg:w-2/5">
     ...
     <div className="max-w-[400px] border-2 rounded-2xl p-6 bg-slate-50">
      ...
      <Link href={'/checkout'}>
      <button className="w-full active:scale-95 bg-primary text-white font-semibold py-2 px-4 rounded-lg mt-5 hover:bg-sky-700">PROCEED TO CHECKOUT</button>
      </Link>
     </div>
