...
const Banner = () => {
  ...
  return (
    ...
    <div className="container flex justify-center  px-4 z-10 mt-28">
      <div className="max-w-2xl text-white">
        ...
        <div className="relative bg-white rounded-full shadow-lg">
          ...
          <button className="absolute  right-2 top-1/2 -translate-y-1/2 bg-black text-white p-3 rounded-full hover:bg-primary/90 transition-colors">
           ...
           {/* search result  */}
       <div className='w-[93%] left-[3.5%] h-full   absolute'>
     {
      data?.result?.map((medicine: IMedicine) =>
       <Link href={`/medicine/:${medicine._id}`} key={medicine._id}>
        <div  className='px-5 py-1 bg-white text-gray-700 border flex  justify-between gap-6'>
      <div className='flex item-stretch gap-3'>
      <Image src={medicine.image} width={50} height={50} className='w-12 h-8' alt={medicine.name}/>  
